...
Demostrar un flujo realista de soporte empresarial donde un host LLM:
...
Para agregar una nueva capacidad de negocio al agente:

1. Crear/ajustar endpoint en `internal_api/routes/` (+ schema si aplica).
2. Exponer llamada en `mcp_server/internal_api_client.py`.
3. Agregar tool en `mcp_server/tools/` con permiso requerido.
4. Ajustar formatting en `mcp_server/formatters.py`.
5. Validar con tests y una conversación end-to-end desde `client/agent_host.py`.

## Estado del proyecto

Consulta `docs/roadmap.md` para el plan por fases (fundación, API interna, MCP, host LLM, seguridad/observabilidad).
...
Si eres nuevo en el repo, empieza por:
1) `app/config.py`, 2) `client/agent_host.py`, 3) `mcp_server/tools/tickets.py`, 4) `internal_api/routes/tickets.py`.
